...
interface NormalizedById {
    [key: string]: UsersData
}
...
const normalizeData = (unnormalizedData: UsersData[]): NormalizedData => {
    const normalizedData: NormalizedData = { byId: {}, allIds: [] }
    unnormalizedData.map((item) => {
        normalizedData.allIds.push(item.id)
        normalizedData.byId[item.id]={id: item.id, title: item.title, body: item.body}
    })
